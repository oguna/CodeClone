...
public class CalculateSimilarity {
	...
	public void execute(int current_revision_num){
		...
		for (CodeFragment after_fix_codeFragment : list_after_fix) {
			...
			if(delete_count.size()>0 && before_fix_count == 1){
				file_writer2(after_fix_codeFragment,before_fix_codeFragment,delete_count,current_revision_num);
			}
			before_fix_count = 0;
			delete_count.clear();
		}
	...
	private boolean similarityCompare(List<String> list,List<String> list2){
		if(list.size() < 20 || list2.size() < 20) return false;
	...
	//いったんファイル生成
	private void file_writer(List<Integer> delete_count, CodeFragment add_codeFragment,
	...
	private void file_writer2(CodeFragment after_fix_codeFragment,CodeFragment before_fix_codeFragment,
			List<Integer> delete_count, int current_revision_num) {
		try{
			File file = new File("F:/tmp/" + current_revision_num + "_" +after_fix_codeFragment.getId() + ".txt");
			PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(file)));
